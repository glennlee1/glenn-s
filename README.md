...
一个零依赖的个人搜索与网址导航首页，已适配桌面、平板和手机。页面支持 15 个搜索入口、网址直达、浅色/深色模式，以及在页面内维护“常用软件”。

线上地址：<https://glennlee1.github.io/glenn-s/>

## 修改站名和默认常用软件

编辑 `config.js` 可以更换站名和恢复默认时使用的推荐清单。发布后，也可以直接点击“常用软件”旁的铅笔按钮，在页面内新增、编辑、删除或恢复默认。
...
每项只需要 `name` 和以 `http://` 或 `https://` 开头的 `url`。无效网址会被自动忽略。页面内的编辑保存在当前浏览器的 localStorage 中，不会自动同步到其他设备。
...
- `app.js`：渲染、搜索、搜索引擎切换、主题和“常用软件”编辑器逻辑
